...
wm.helper.getKeys = function(obj)
{
    var keys = {};
...
wm.helper.makeToJSON = function(obj, excl)
{
    return function()
    {                
        var tmp = {}
        var keys = wm.helper.getKeys(this);

        for (var x in keys) 
            if (!(x in excl) || ((x in excl) && excl[x]==false))
                tmp[x] = this[x]; 
...
wm.helper.toUTF8 = function(str)
{
    return unescape(encodeURIComponent(str));
};

wm.helper.fromUTF8 = function(str)
{
...
wm.deviceready = function (event)
{
    ...
    for (var iModule in wm.bridge.m_modules)
    {
        // device needs to go last.  We could add dependencies to the registerModule function
        // but since this is the only module with a dependency so far we might as well hard code.
        if (wm.bridge.m_modules[iModule] === "device")
            continue;
            
        wm.bridge.exec(null, wm.bridge.m_modules[iModule], "onReady", {});
    }
    // Signal to the bridge that we're ready    
    wm.bridge.exec(null, "device", "onReady", {});
};
...
wm.Bridge.prototype.fireEvent = function(obj)
{
    ...
    // and also delete device ready event listeners as they should all be one
    // shot.
    var isDeviceReadyEvent = obj.eventName === "deviceready"
    if (isDeviceReadyEvent)
    {
    ...
    if (isDeviceReadyEvent)
    {
        this.m_eventTable[obj.eventName] = [];
    }
    return;
};
...
wm.Bridge.prototype.registerModule = function(name, dependencies)
{
    if (typeof dependencies !== 'undefined' && dependencies instanceof Array)
        for (var i=0; i<dependencies.length; i++)
            this.registerModule(dependencies[i]);

    if (this.m_modules.indexOf(name) !== -1)
    {
        wm.error("Can't register module as '" + name + "' is already defined");
        return;
    }
    
    this.m_modules.push(name);
    this.exec(null, name, 'registerModule', {});    
}

wm.Bridge.prototype.unregisterModule = function(name)
{
    var index = this.m_modules.indexOf(name);
    if (index === -1)
    {
        wm.error("Can't unregisterModule module as '" + name + "' is not defined");
        return;
    }
    
    this.m_modules.splice(index, 1);
    this.exec(null, name, 'unregisterModule', {});    
}
...
wm.bridge.registerModule("events");
//
// accelerometer API
//
...
wm.bridge.registerModule("accelerometer");
//
// camera API
//
...
wm.bridge.registerModule("camera");
//
// capture API
//
...
wm.bridge.registerModule("capture");
//
// childBrowser API
//
...
wm.bridge.registerModule("childBrowser");
//
// compass API
//
...
wm.bridge.registerModule("compass");
//
// connection API
//
...
wm.bridge.registerModule("connection");
//
// device API
//
wm.bridge.registerEvent('deviceready','device');
...
wm.bridge.registerModule("device");
//
// file API
//
...
FileReader.prototype.toJSON = wm.helper.makeToJSON(FileReader, {"readyState":true, "result":true, "error":true, "onloadstart":true, "onprogress":true, "onload":true, "onabort":true, "onerror":true, "onloadend":true});
...
wm.bridge.registerModule("geolocation");
//
// media API
//
...
wm.bridge.registerModule("media");
//
// notification API
//
...
navigator.notification.addLogFile = function(chan, filename)
{
    // Check for required parameters
    if (typeof chan === "undefined")
    {
        wm.error("Parameter chan is undefined");
        return false;
    }
    if (typeof filename === "undefined")
    {
        wm.error("Parameter filename is undefined");
        return false;
    }

    // Check parameter types and default unsupplied ones
    if (chan !== null && typeof chan != 'string')
    {
        wm.error("Parameter chan must be a string or null");
        return false;
    }
    ...
    {
        wm.error("Parameter filename must be a string or null");
        return false;
    }
    ...
    var proxy = new Notification.Proxy(this, {});
    proxy.method = 'addLogFile';
    proxy.params = {chan:chan, filename:filename};

    if (typeof proxy._addLogFile == 'function')
        proxy._addLogFile();
    wm.bridge.exec(null, 'notification', 'addLogFile', proxy.params);

    return true;
};

navigator.notification.log = function(chan, message)
{
    // Check for required parameters
    if (typeof chan === "undefined")
    {
    ...
    var proxy = new Notification.Proxy(this, {});
    proxy.method = 'log';
    proxy.params = {chan:chan, message:message};

    if (typeof proxy._log == 'function')
        proxy._log();
    wm.bridge.exec(null, 'notification', 'log', proxy.params);

    return true;
};

navigator.notification.startLog = function()
{
    // Check for required parameters
    ...
    var proxy = new Notification.Proxy(this, {});
    proxy.method = 'startLog';
    proxy.params = {};

    if (typeof proxy._startLog == 'function')
        proxy._startLog();
    wm.bridge.exec(null, 'notification', 'startLog', proxy.params);

    return true;
};

navigator.notification.endLog = function(success)
{
    // Check for required parameters
    if (typeof success === "undefined")
    {
        wm.error("Parameter success is undefined");
        return false;
    }
    ...
    {
        wm.error("Parameter success must be a boolean or null");
        return false;
    }
    ...
    var proxy = new Notification.Proxy(this, {});
    proxy.method = 'endLog';
    proxy.params = {success:success};

    if (typeof proxy._endLog == 'function')
        proxy._endLog();
    wm.bridge.exec(null, 'notification', 'endLog', proxy.params);
...
wm.bridge.registerModule("notification");
